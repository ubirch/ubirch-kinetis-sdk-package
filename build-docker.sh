...
function build_software() {
  docker run --user `id -u`:`id -g` --workdir=/build/ubirch-kinetis-sdk-package --volume=${PWD}/..:/build --env=${SDK_ROOT}  --env=${TOOLCHAIN} --entrypoint=/bin/bash ubirch/arm-build:${ARM_CONTAINER_VERSION} ./build.sh
  if [ $? -ne 0 ]; then
